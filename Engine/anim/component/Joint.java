...
public class Joint {
	 ...
	 *                           position.
	 */
	public Joint(byte index, String name, Matrix4f bindLocalTransform) {
		this.index = index;
		this.name = name;
	...
	public void calcInverseBindTransform(Matrix4f parentBindTransform) {
		final Matrix4f bindTransform = Matrix4f.mul(parentBindTransform, localBindTransform, null);
		Matrix4f.invert(bindTransform, inverseBindTransform);
		for (final Joint child : children) {
