...
import map.Chunk;
import procedural.InfiniteVoronoi;
...
public class Voronoi {
	...
	private final float minDistanceBetweenSites;
	private int PQcount;
	private int PQmin;
	private int PQhashsize;

	private Halfedge PQhash[];
	private final HashMap<Long, List<IntersectionGraphEdge>> intersections;
	...
	public Voronoi(float minDistanceBetweenSites) {
		...
		intersections = new HashMap<Long, List<IntersectionGraphEdge>>();
	}
	...
	/*
	 * This delete routine can't reclaim node, since pointers from hash table may be
	 * present.
	 */
	 ...
	 * @return
	 */
	public List<GraphEdge> generateVoronoi(float[] xValuesIn, float[] yValuesIn, int realSize, float minX, float maxX,
		...
		// Get cells
		cells = new Cell[xValuesIn.length];

		for (int i = 0; i < xValuesIn.length; i++) {
			cells[i] = new Cell();
		}

		sort(xValuesIn, yValuesIn, realSize);
	...
	public List<IntersectionGraphEdge> getIntersectionPts(int x, int y) {
		return intersections.get(NoiseUtil.szudzik(x, y));
	}
	...
	private void markPts(int x, int y, float x1, float y1, GraphEdge edge) {
		final int p1x = Math.floorDiv(x, Chunk.CHUNK_SIZE), p1y = Math.floorDiv(y, Chunk.CHUNK_SIZE);
		final long key = NoiseUtil.szudzik(p1x, p1y);
		final IntersectionGraphEdge pt = new IntersectionGraphEdge(x1, y1, edge);

		List<IntersectionGraphEdge> edges = intersections.get(key);
		if (edges != null) {
			if (!edges.contains(pt)) {
				edges.add(pt);
			}
		} else {
			edges = new ArrayList<IntersectionGraphEdge>();
			edges.add(new IntersectionGraphEdge(x, y, null));// center
			edges.add(pt);
			intersections.put(key, edges);
		}
	}
	...
	private void pruneIntersections() {
		final List<Long> removeThese = new ArrayList<Long>();

		for (final long l : intersections.keySet()) {
			if (intersections.get(l).size() < 4) {
				removeThese.add(l);
			}
		}

		for (final long l : removeThese) {
			intersections.remove(l);
		}
	}

	private void pushGraphEdge(Site leftSite, Site rightSite, float x1, float y1, float x2, float y2) {
		/*
		 * if (x1 == InfiniteVoronoi.minX || x2 == InfiniteVoronoi.minX || y1 ==
		 * InfiniteVoronoi.minY || y2 == InfiniteVoronoi.minY) { return; }
		 * 
		 * int mx = (InfiniteVoronoi.minX + (InfiniteVoronoi.scaledSize)); int my =
		 * (InfiniteVoronoi.minY + (InfiniteVoronoi.scaledSize)); if (x1 == mx || x2 ==
		 * mx || y1 == my || y2 == my) { return; }
		 */
		...
		final int p1x = (int) x1, p1y = (int) y1, p2x = (int) x2, p2y = (int) y2;

		final Vector2f d = new Vector2f(x2 - x1, y2 - y1);
		d.normalize();
		d.mul(InfiniteVoronoi.INTERSECTION_SIZE);
		newEdge.x1 += d.x;
		newEdge.y1 += d.y;
		newEdge.x2 -= d.x;
		newEdge.y2 -= d.y;

		markPts(p1x, p1y, newEdge.x1, newEdge.y1, newEdge);
		markPts(p2x, p2y, newEdge.x2, newEdge.y2, newEdge);

		final Cell c1 = cells[newEdge.site1];
		final Cell c2 = cells[newEdge.site2];
		c1.center = leftSite.coord;
		c2.center = rightSite.coord;
	...
	private void sort(float[] xValuesIn, float[] yValuesIn, int count) {
		...
		final float sn = (float) nsites + 4;
		sqrt_nsites = (int) Math.sqrt(sn);

		// Copy the inputs so we don't modify the originals
		final float[] xValues = new float[count];
		final float[] yValues = new float[count];
	 ...
	 * deltay (can all be estimates). Performance suffers if they are wrong; better
	 * to make nsites, deltax, and deltay too big than too small. (?)
	 */
